                          ...
                          void *event_data)
{
    ...
    switch (event_id)
    {
        ...
        case WIFI_EVENT_STA_CONNECTED:
            ...
            ESP_LOGI(TAG, " SSID: %d", event_sta_connected->aid);
            ...
            // Set up the WiFi interface and start DHCP process (s4.2)
                        ...
                        void *event_data)
{
    ...
    switch (event_id)
    {
        ...
        case IP_EVENT_STA_GOT_IP:
            ...
            // Set IPv4_OBTAINED bit in the event group
                       ...
                       void *data)
{
    ...
    // (s1.3) Get esp_netif driver handle
 ...
 */
 ...
 esp_err_t wifi_sta_init(EventGroupHandle_t event_group) 
 {
    ...
    // Note: You can save these settings in NVS for future use if you are 
    // using a provisioning app to set up the device or fast reconnect
 ...
 esp_err_t wifi_sta_stop(void) {
    ...
    // Unregister WiFi event: station stop
    esp_ret = esp_event_handler_unregister(WIFI_EVENT, 
                                         WIFI_EVENT_STA_STOP,
                                         &on_wifi_event);
    if (esp_ret != ESP_OK) {
    ...
    // Unregister WiFi event: station connected
    esp_ret = esp_event_handler_unregister(WIFI_EVENT, 
                                         WIFI_EVENT_STA_CONNECTED,
                                         &on_wifi_event);
    if (esp_ret != ESP_OK) {
    ...
    // Unregister WiFi event: station disconnected
    esp_ret = esp_event_handler_unregister(WIFI_EVENT, 
                                         WIFI_EVENT_STA_DISCONNECTED,
                                         &on_wifi_event);
    if (esp_ret != ESP_OK) {
...
#if CONFIG_WIFI_STA_CONNECT_IPV4 || CONFIG_WIFI_STA_CONNECT_UNSPECIFIED
    // Unregister IP event: station got ipv4 address
    ...
    // Unregister IP event: lost IP address
    esp_ret = esp_event_handler_unregister(IP_EVENT,
                                         IP_EVENT_STA_LOST_IP,
                                         &on_ip_event);
    if (esp_ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to unregister lost ip event handler");
        return ESP_FAIL;
    }
    ...
    // (s8.3) Deinitialize the WiFi driver, free resources
    esp_ret = esp_wifi_deinit();
    if (esp_ret == ESP_ERR_WIFI_NOT_INIT) {
        ESP_LOGI(TAG, "WiFi driver already deinitialized");
    } else if (esp_ret != ESP_OK) {
        ESP_LOGE(TAG, "Error (%d): Failed to deinitialize Wifi driver", esp_ret);
        return ESP_FAIL;
    }
 ...
 bool wait_for_wifi(EventGroupHandle_t wifi_event_group, int32_t timeout_sec) 
 {
    ...
    wifi_event_bits = xEventGroupWaitBits(wifi_event_group,
                                          WIFI_STA_CONNECTED_BIT,
                                          pdFALSE,
                                          pdFALSE,
                                          pdMS_TO_TICKS(timeout_sec * 1000));
    if (wifi_event_bits & WIFI_STA_CONNECTED_BIT) {
        ESP_LOGI(TAG, "Succesfully connected to Wi-Fi network!");
    }
    else {
        ESP_LOGI(TAG, "Failed to connect to Wi-Fi connection");
        return false;
    }
    ...
    wifi_event_bits = xEventGroupWaitBits(wifi_event_group,
                                          ...
                                          pdFALSE,
                                          pdMS_TO_TICKS(timeout_sec * 1000));
    if ((wifi_event_bits & WIFI_STA_IPV4_OBTAINED_BIT) ||
        (wifi_event_bits & WIFI_STA_IPV6_OBTAINED_BIT)) {
        ESP_LOGI(TAG, "Succesfully got IP address!");
    }
    else {
